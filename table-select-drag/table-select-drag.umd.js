(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["table-select-drag"] = factory(require("vue"));
	else
		root["table-select-drag"] = factory(root["Vue"]);
...
// CONCATENATED MODULE: ./src/package/table-drag/index.js
 ...
 * 按住shift支持断序拖拉多选
 * */
/* harmony default export */ var table_drag = ({
  inserted: function inserted(el, binding, vNode) {
    // 获取自定义指令绑定区域DOM，在此区域监听用户交互操作
    var oDiv = el; // 渲染数组名称

    var tableDataName = 'tableData'; // 多选集合数组名称

    var changesListName = 'multipleSelection'; // 自定义指令接收表格渲染数组名称、多选集合数组名称
    ...
    var tableDom = '';
    var searchDom = ''; // 分别获取表格组件、条件搜索区域组件

    setTimeout(function () {
      // 插槽内dom和外层渲染时间不一致，无法实时拿到，不加定时器拿不到（针对项目中条件搜索部分）
      vNode.context.$children.forEach(function (item) {
    ...
    var onShift = false; // 监听键盘shift事件，用于开启、关闭断序多选功能
    ...
    var selList; // 用于存储鼠标，进行初始化鼠标位置记录，用于渲染动态选框
    ...
    oDiv.onmousedown = function (ev) {
      // 移动事件标识，区分移动、点击事件
      var moveFlag = false; // selList是否初始化完毕标识（用于限制selList中tr每次只初始化一次）

      var flag = true;
      selList = []; // 获得指令下的dom对应的表格

      var fileNodes = oDiv.getElementsByTagName('tr'); // 获取鼠标信息

      evt = window.event || arguments[0]; // 记录鼠标位置
      ...
      * */
      ...
      var tableHeader = oDiv.getElementsByClassName('el-table__header-wrapper')[0]; // top = oDiv.offsetTop + searchDom.clientHeight + tableHeader.clientHeight

      top = oDiv.offsetTop + tableHeader.clientHeight;
      left = oDiv.offsetLeft; // 创建拖拉选框div
      ...
      document.onmousemove = function (ev) {
        ...
        _y = evt.y || evt.clientY; // 获取表格内容滚动距离,适配固定选框位置
        ...
        * */
        ...
        if (reFlag) {
          ...
          timer = setTimeout(function () {
            ...
            for (var _i = 0; _i < selList.length; _i++) {
              /*
              ...
              * */
              if (!vNode.context[tableDataName][_i]) {
                break;
              }
              /*
              ...
              * */
              ...
              var sl = selList[_i].offsetWidth + selList[_i].offsetLeft;
              var st = selList[_i].offsetHeight + selList[_i].offsetTop;

              if (sl > _l && st > _t && selList[_i].offsetLeft < _l + _w && selList[_i].offsetTop < _t + _h) {
                // 在选框中的tr
                if (vNode.context[tableDataName][_i]) {
                  // 修改tr为选中状态
                  tableDom.toggleRowSelection(vNode.context[tableDataName][_i], true); // 向多选数据源添加选中数据（会添加重复数据，需要去重）

                  vNode.context[changesListName].push(vNode.context[tableDataName][_i]);
                }
              } else {
                // 根据shift按下事件选择是否清空已选择的数据
                if (!onShift) {
                  // 未按下shift，则每次都是重新选中，将为选中数据选中状态取消，并且剔除出多选数据源
                  tableDom.toggleRowSelection(vNode.context[tableDataName][_i], false);
                  vNode.context[changesListName].forEach(function (ele, i) {
      ...
      document.onmouseup = function () {
        // 把鼠标移动事初始化
        document.onmousemove = null; // 释放定时器

        clearTimeout(timer); // 清除选框DOM
...
external_commonjs_vue_commonjs2_vue_root_Vue_default.a.directive('table-frag', table_drag);
...
//# sourceMappingURL=table-select-drag.umd.js.map